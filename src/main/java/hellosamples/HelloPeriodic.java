...
/**
 * Sample Temporal workflow that demonstrates periodic workflow execution with a
 * random delay. To
 * learn about periodic execution with a fixed delay (defined by a cron), check
 * out the {@link
 * HelloCron} example.
 */
public class HelloPeriodic {
  ...
  public interface GreetingWorkflow {
    ...
    /**
     * Users will invoke this signal when they want the main workflow loop to
     * complete.
     */
    @SignalMethod
    void requestExit();
  ...
  // Define the workflow implementation which implements the greetPeriodically
  // workflow method.
  public static class GreetingWorkflowImpl implements GreetingWorkflow {

    // In this example, we use an internal of 5 seconds with an intended variation
    // of +/- 1 second
    // between executions of some useful work. In real applications a higher value
    // may be more
    // appropriate, for example one that matches a business cycle of several hours
    // or even days.
    ...
    // More details:
    // https://docs.temporal.io/docs/java/workflows/#large-event-histories
     ...
     * invocations that
     * are executed outside the workflow thread on the activity worker, that can be
     * on a different
     * host. Temporal is going to dispatch the activity results back to the workflow
     * and unblock the
  ...
  public static void main(String[] args) throws InterruptedException {
    ...
    try {
      ...
      // If the workflow is already running, we cannot start it. Instead, we will
      // connect to the
      // existing instance.
    ...
    // The workflow is running now. In this example, we pause for a few seconds to
    // observe its
    // output.
